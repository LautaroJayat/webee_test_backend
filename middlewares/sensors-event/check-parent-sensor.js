...
const checkValidParentSensorBeforeUpdate = async (req, res, next) => {
  const { _id } = req.params;
  ...
  if (!_id || _id === "") {
    //console.log("id", _id);
    return res.status(400).json({ message: "Could not find sensorEvent id" });
  }
  try {
...
module.exports = { checkValidParentSensor, checkValidParentSensorBeforeUpdate };
